// Duplicated from client/src/lib/parseShotText.ts so the Siri Shortcut
// endpoint (POST /api/shots/from-text, see routes/shotsFromText.ts) can run
// the same dictated-text parsing server-side, without a browser. Keep this
// file in sync with the client copy by hand — see the golf-app migration
// plan for why a shared workspace package wasn't set up for this yet.
//
// Types here are plain strings (not literal unions imported from
// client/src/types/models.ts) to avoid a cross-workspace import; the values
// used are identical to the client's ShotDirection/ShotLie/ShotStrength.
